...
interface RequestOptions {
  ...
  /**
   * When set, HMAC-sign the request body with this key and attach
   * X-Execlave-Timestamp + X-Execlave-Signature headers (defense-in-depth +
   * replay protection). The server verifies with the same key (the API key).
   */
  sign?: { key: string };
}
...
export async function request(opts: RequestOptions): Promise<HttpResponse> {
  const { method, url, headers = {}, body, timeout = 30_000, resolveOnClientError = false, sign } =
    opts;
  ...
  return new Promise((resolve, reject) => {
    ...
    // HMAC-sign the exact body bytes the server will receive. The signed string
    // is `${timestamp}.${rawBody}` — identical to the backend's verification.
    if (sign?.key) {
      const timestamp = Math.floor(Date.now() / 1000).toString();
      const signature =
        'sha256=' +
        createHmac('sha256', sign.key)
          .update(`${timestamp}.${bodyStr ?? ''}`)
          .digest('hex');
      reqHeaders['X-Execlave-Timestamp'] = timestamp;
      reqHeaders['X-Execlave-Signature'] = signature;
    }
