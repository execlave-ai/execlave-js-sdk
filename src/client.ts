...
export class Execlave {
  ...
  private _policyCacheKey(
    ...
    // included or not. Non-serializable metadata still gets a distinct,
    // deterministic marker rather than crashing cache-key construction.
    let extra: string;
    try {
      extra = JSON.stringify([metadata ?? null, estimatedCost ?? null, tools ?? null]);
    } catch {
