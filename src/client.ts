...
export class Execlave {
  ...
  async enforcePolicy(opts: EnforcePolicyOptions): Promise<EnforceResult> {
    ...
    // 1. Check cache
    const cacheKey = this._policyCacheKey(opts.agentId, opts.input, effectiveEnvironment);
  ...
  private _policyCacheKey(agentId: string, input: string, environment: string): string {
    const hash = createHash('sha256')
      .update(`${environment}:${agentId}:${input}`)
      .digest('hex')
      .slice(0, 16);
    return `policy:${hash}`;
  }
    ...
    while (Date.now() - startedAt < timeoutMs) {
      ...
      if (approval.status === 'approved') {
        return {
          allowed: true,
          approvalRequestId,
          ...(approval.certificate !== undefined ? { certificate: approval.certificate } : {}),
        };
      }
  ...
  private _log(msg: string, ...args: unknown[]): void {
    if (this._debug) {
      // Constant format string: `msg`/`args` are passed as data, never as a
      // format specifier, so externally-influenced text cannot forge log
      // output via injected %-specifiers (CWE-134).
