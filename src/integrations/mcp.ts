  ...
  const wrapped = async (request: any, ...rest: any[]): Promise<any> => {
    ...
    if (enforce && name) {
      try {
        ...
        // classifier read, not a governance-complete record. The
        // certificate's action-binding digest covers the WHOLE sealed action
        // context, so the untruncated arguments are sealed into metadata via
        // the shared _actionBinding helper (see its module doc for why this
        // is centralized) too: a change past input's truncation point must
        // still invalidate the certificate, not silently escape it.
        await enforcePolicyBound(exe, {
          agentId: opts.agentId,
          input: truncateForClassifier(args) ?? `tool:${name}`,
          tools: [name],
...
function extractContent(content: unknown): string | null {
  if (!Array.isArray(content)) return truncateForClassifier(content);
  const parts: string[] = [];
  for (const item of content) {
    const text = (item as any)?.text;
    if (typeof text === 'string') parts.push(text);
    else {
      const s = truncateForClassifier(item);
      if (s) parts.push(s);
    }
  }
