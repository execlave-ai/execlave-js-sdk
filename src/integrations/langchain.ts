...
export class ExeclaveCallbackHandler {
  ...
  private _toolMeta = new Map<string, { toolName: string; input: unknown }>();
  ...
  async handleChainStart(
    ...
    runId: string,
    // The trailing parameters are read positionally by LangChain, and their
    // ORDER CHANGED between major versions:
    //
    //   0.3.x  (…, runId, parentRunId?, tags?, metadata?, runType?, runName?)
    //   1.x    (…, runId, runType?, tags?, metadata?, runName?, parentRunId?, extra?)
    //
    // Binding `parentRunId` to the 4th position — correct for 0.3 — silently
    // receives `runType` (the string "chain") under 1.x. That is not a cosmetic
    // break: `!parentRunId` would then be false for every root chain, so
    // chain-level enforcement would never run, and the trace tree would be
    // parented to a non-existent run. Resolve it by shape instead of position.
    ...rest: unknown[]
  ): Promise<void> {
    const parentRunId = resolveParentRunId(rest);
  ...
  async handleToolStart(
    ...
    this._toolMeta.set(runId, { toolName, input });
  ...
  async handleToolEnd(output: unknown, runId: string): Promise<void> {
    const meta = this._toolMeta.get(runId);
    this._toolMeta.delete(runId);
    if (this._enforce && meta && output !== undefined && output !== null) {
      await this._exe.enforceToolOutput({
        agentId: this._agentId,
...
/**
 * LangChain's closed vocabulary for `runType`. A parent run id is never one of
 * these, which is what makes the two call shapes below separable.
 */
...
/** LangChain run ids are UUIDs; `runName` is a human label and is not. */
const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

/**
 * Recover `parentRunId` from `handleChainStart`'s trailing arguments across
 * LangChain major versions, which pass it at different positions:
 *
 ...
 *
 * The first trailing slot decides which shape this is: under 0.3 it holds the
 * parent run id, under 1.x it holds `runType`. Since `runType` comes from a
 * closed vocabulary and a run id never does, a string there that is not a known
 * run type is a parent run id — which also keeps non-UUID run ids working.
 *
 * When that slot is a run type (1.x) or absent (a 0.3 root chain), the 1.x
 * parent slot is consulted, and there a UUID shape is required: under 0.3 the
 * same index holds `runName`, and mistaking a chain's display name for a parent
 * id would suppress enforcement on a root chain.
 *
 * Every ambiguous case therefore resolves to `undefined`, which the handler
 * reads as "root chain — enforce it". Misidentification fails toward enforcing
 * rather than skipping.
 */
export function resolveParentRunId(rest: unknown[]): string | undefined {
  const first = rest[0];
  if (typeof first === 'string' && !RUN_TYPES.has(first)) return first;

  const atV1 = rest[4];
  if (typeof atV1 === 'string' && UUID_RE.test(atV1)) return atV1;
  return undefined;
}
