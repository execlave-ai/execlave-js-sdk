...
import { truncateForClassifier, sealMetadataEntry, enforcePolicyBound } from './_actionBinding';
...
export class ExeclaveTracingProcessor {
  ...
  private async _enforceTool(toolName: string, args: unknown): Promise<void> {
    ...
    try {
      await enforcePolicyBound(this._exe, {
        ...
        input: truncateForClassifier(args) ?? `tool:${toolName}`,
        tools: [toolName],
        metadata: sealMetadataEntry('toolArguments', args),
      });
    } catch (err) {
      if (isEnforcementError(err)) throw err;
      // eslint-disable-next-line no-console
  ...
  private async _enforceInput(kind: string, name: string, input: unknown): Promise<void> {
    if (!this._enforce) return;
    const inputStr = truncateForClassifier(input);
    if (!inputStr) return;
    try {
      await enforcePolicyBound(this._exe, {
        agentId: this._agentId,
        input: inputStr,
        metadata: sealMetadataEntry('spanInput', input),
      ...
      if (isEnforcementError(err)) throw err;
      // eslint-disable-next-line no-console
      console.warn(`[execlave] ${kind}:${name} input enforcement failed (non-fatal):`, err);
    }
  }
  ...
  private async _startSpan(span: any): Promise<void> {
    ...
    // Enforcement runs before the span opens so a block aborts the SDK run
    // without producing an orphaned span. Tool spans check tool-allowlist
    // policies; agent/generation spans check injection/content policies on
    // the input (where prompt injection arrives).
    if (kind === SPAN_KIND_TOOL) {
      const args = getAttr(spanData, 'input') ?? getAttr(spanData, 'arguments');
      await this._enforceTool(name, args);
    } else if (kind === SPAN_KIND_AGENT || kind === SPAN_KIND_LLM) {
      const input = getAttr(spanData, 'input') ?? getAttr(spanData, 'messages') ?? getAttr(spanData, 'prompt');
      if (input) {
        await this._enforceInput(kind, name, input);
      }
    }
  ...
  private async _endSpan(span: any): Promise<void> {
    const spanId = getAttr(span, 'spanId') ?? getAttr(span, 'id');
    if (!spanId) return;
    ...
    // Tool output enforcement: scan the result before it feeds back to the
    // model. A block-mode tool_output_scan policy throws PolicyBlockedError,
    // preventing poisoned/PII-laden tool output from reaching the LLM.
    const dataClass = spanData ? (spanData as { constructor?: { name?: string } }).constructor?.name ?? '' : '';
    const kind = KIND_MAP[dataClass] ?? SPAN_KIND_AGENT;
    if (this._enforce && kind === SPAN_KIND_TOOL && output) {
      const toolName = spanName(spanData) ?? 'unknown_tool';
      const input = getAttr(spanData, 'input') ?? getAttr(spanData, 'arguments');
    ...
    if (error) {
      const msg = (error as { message?: string }).message ?? truncateForClassifier(error) ?? 'error';
