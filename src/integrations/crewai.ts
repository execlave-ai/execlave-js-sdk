...
async function recordStep(
  ...
  if (isTool && opts.enforce !== false) {
    try {
      // `input` is a bounded view (truncateForClassifier) for policy/
      // classifier read. The certificate's action-binding digest covers the
      // WHOLE sealed action context, so the untruncated tool input is
      // sealed into metadata via the shared _actionBinding helper too: a
      // change past input's truncation point must still invalidate the
      // certificate, not silently escape it.
      await enforcePolicyBound(exe, {
        agentId: opts.agentId,
        input: truncateForClassifier(toolInput) ?? `tool:${tool}`,
        tools: [String(tool)],
        metadata: sealMetadataEntry('toolInput', toolInput),
      });
    } catch (err) {
      if (isEnforcementError(err)) throw err;
      // eslint-disable-next-line no-console
  ...
  if (isTool && opts.enforce !== false && output !== null && output !== undefined) {
    await exe.enforceToolOutput({
      agentId: opts.agentId,
