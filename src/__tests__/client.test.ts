...
describe('Execlave Client', () => {
  ...
  describe('enforcePolicy outage & cache safety', () => {
    it('fail_closed throws on the FIRST network failure, not after the breaker trips', async () => {
      const exe = createClient({ enforcementOnOutage: 'fail_closed' });
      mockRequest.mockRejectedValueOnce(new Error('ECONNREFUSED'));
      await expect(exe.enforcePolicy({ agentId: 'a', input: 'x' })).rejects.toBeInstanceOf(
        EnforcementUnavailableError,
      );
    });

    it('fail_open allows on a network failure and marks the result source', async () => {
      const exe = createClient(); // default fail_open
      mockRequest.mockRejectedValueOnce(new Error('ECONNREFUSED'));
      const r = await exe.enforcePolicy({ agentId: 'a', input: 'x' });
    ...
    it('still throws a plain error for a 4xx client mistake (not an outage)', async () => {
      const exe = createClient({ enforcementOnOutage: 'fail_open' });
    ...
    it('does NOT serve a cached allow once the agent is paused (kill switch)', async () => {
      ...
      await exe.enforcePolicy({ agentId: 'a', input: 'same-input' });
      expect(mockRequest).toHaveBeenCalledTimes(1);

      // Kill switch fires (control channel would set this).
      (exe as any)._state = 'PAUSED';
      ...
      await expect(exe.enforcePolicy({ agentId: 'a', input: 'same-input' })).rejects.toBeTruthy();
    ...
    it('sends conversationHistory and keys the cache on it', async () => {
      ...
      mockSuccess({ allowed: true }, 200);
      await exe.enforcePolicy({ agentId: 'a', input: 'now do it', conversationHistory: history });
    ...
    it('omits conversationHistory from the payload when not provided', async () => {
      const exe = createClient();
      mockSuccess({ allowed: true }, 200);
      await exe.enforcePolicy({ agentId: 'a', input: 'hi' });
    ...
    it('enforceToolOutput sends toolOutputs and returns the decision', async () => {
      const exe = createClient();
      mockSuccess({ allowed: true }, 200);
      const r = await exe.enforceToolOutput({
        agentId: 'a',
        toolName: 'web_search',
    ...
    it('enforceToolOutput throws PolicyBlockedError when a tool_output_scan policy blocks', async () => {
      ...
      await expect(
        exe.enforceToolOutput({ agentId: 'a', toolName: 'web_search', output: 'SSN 123-45-6789' }),
      ).rejects.toBeTruthy();
    });
  });
  ...
  describe('constructor', () => {
    it('should accept apiKey from config', () => {
      ...
      expect(exe).toBeInstanceOf(Execlave);
    });
    ...
    it('should use custom baseUrl', () => {
      const exe = createClient({ baseUrl: 'https://api.example.com/' });
    ...
    it('should upgrade cloud HTTP baseUrl to HTTPS', async () => {
      const exe = createClient({ baseUrl: 'http://api.execlave.com/' });
      mockSuccess({ allowed: true });

      await exe.enforcePolicy({ agentId: 'my-agent', input: 'hello' });
  ...
  describe('registerAgent()', () => {
    it('should POST agent with correct payload and return Agent', async () => {
      ...
      const agent = await exe.registerAgent({
        agentId: 'my-bot',
        name: 'My Bot',
      });
    ...
    it('should omit autonomyLevel from the payload when not provided', async () => {
      ...
      await exe.registerAgent({ agentId: 'plain-bot', name: 'Plain Bot' });
    ...
    it('should handle already-exists by searching and returning existing agent', async () => {
      ...
      const agent = await exe.registerAgent({ agentId: 'dup-bot', name: 'Dup Bot' });
    ...
    it('should reject a list-envelope response without the requested agent', async () => {
      ...
      await expect(
        exe.registerAgent({ agentId: 'my-bot', name: 'My Bot' }),
    ...
    it('should rethrow non-duplicate errors', async () => {
      ...
      await expect(
        exe.registerAgent({ agentId: 'fail-bot', name: 'Fail Bot' }),
  ...
  describe('getAgentCredential()', () => {
    ...
    it('should POST to the credential endpoint and cache the result', async () => {
      ...
      const credential = await exe.getAgentCredential('33333333-3333-3333-3333-333333333333');
    ...
    it('should return the cached credential until 60 seconds before expiry', async () => {
      ...
      const first = await exe.getAgentCredential('agent-uuid');
      const second = await exe.getAgentCredential('agent-uuid');
    ...
    it('should cache credentials separately by agentId', async () => {
      ...
      const first = await exe.getAgentCredential('agent-a');
      const second = await exe.getAgentCredential('agent-b');
      const firstCached = await exe.getAgentCredential('agent-a');
  ...
  describe('reportAgentMetadata()', () => {
    it('POSTs to the version endpoint using the cached agent UUID', async () => {
      ...
      await exe.registerAgent({ agentId: 'deploy-bot', name: 'Deploy Bot' });

      mockSuccess({ id: 'ver-1', versionNumber: 2 });
      await exe.reportAgentMetadata({
        agentId: 'deploy-bot',
        versionLabel: 'v2.1.0',
        gitCommit: 'abc123',
    ...
    it('looks up the UUID by external id when not cached', async () => {
      ...
      await exe.reportAgentMetadata({ agentId: 'uncached-bot', versionLabel: 'v1' });
  ...
  describe('startTrace()', () => {
    ...
    it('should accept an explicit traceId', () => {
      const exe = createClient();
      const trace = exe.startTrace({ traceId: 'my-trace', agentId: 'bot' });
    ...
    it('should throw ExeclaveError after shutdown', async () => {
      ...
      expect(() => exe.startTrace()).toThrow('SDK has been shut down');
    });
  });
  ...
  describe('enforcePolicy()', () => {
    it('should return allowed result on 200', async () => {
      ...
      const result = await exe.enforcePolicy({ agentId: 'bot-1', input: 'hello' });
    ...
    it('should resolve cached external agentId to UUID before enforcement', async () => {
      ...
      await exe.registerAgent({ agentId: 'my-bot', name: 'My Bot' });
      mockSuccess({ allowed: true }, 200);

      const result = await exe.enforcePolicy({ agentId: 'my-bot', input: 'hello' });
    ...
    it('should poll, auto-verify the certificate, and resolve when approval is granted', async () => {
      ...
      const result = await exe.enforcePolicy({ agentId: 'bot-1', input: 'delete records' });
    ...
    it('should throw CertificateMismatchError when auto-verify returns valid:false', async () => {
      ...
      await expect(
        exe.enforcePolicy({ agentId: 'bot-1', input: 'delete records' }),
    ...
    it('should throw PolicyDeniedError when approval is denied', async () => {
      ...
      await expect(
        exe.enforcePolicy({ agentId: 'bot-1', input: 'dangerous action' }),
    ...
    it('should throw PlanLimitExceededError on 402 when planLimitBehavior is fail_closed', async () => {
      ...
      await expect(
        exe.enforcePolicy({ agentId: 'bot-1', input: 'hello' }),
      ).rejects.toBeInstanceOf(PlanLimitExceededError);

      expect((exe as any)._cbFailures).toBe(0);
      expect((exe as any)._cbOpen).toBe(false);
    });
  });
  ...
  describe('verifyApproval()', () => {
    it('should return valid approval verification result', async () => {
      ...
      const result = await exe.verifyApproval('apr_1', { action: 'delete', count: 100 });
    ...
    it('should return invalid approval verification reason', async () => {
      ...
      const result = await exe.verifyApproval('apr_1', { action: 'delete', count: 101 });
  ...
  describe('flush()', () => {
    ...
    it('should attach a signed agent credential when stampIdentity is enabled', async () => {
      ...
      const exe = createClient({ stampIdentity: true, asyncMode: true, flushIntervalMs: 10_000_000 });
  ...
  describe('PII scrubbing', () => {
    it('should scrub email addresses', async () => {
      const exe = createClient({ privacy: { enabled: true } });
      mockSuccess({}); // flush
    ...
    it('should add pii_detected metadata with hashes', async () => {
      const exe = createClient({ privacy: { enabled: true, hashPii: true } });
      mockSuccess({});
    ...
    it('should not scrub when privacy is disabled', async () => {
      const exe = createClient({ privacy: { enabled: false } });
      mockSuccess({});
  ...
  describe('injection scanning', () => {
    it('should detect "ignore all previous instructions"', async () => {
      const exe = createClient({ enableInjectionScan: true });
      mockSuccess({});
    ...
    it('should not scan when enableInjectionScan is false', async () => {
      const exe = createClient({ enableInjectionScan: false });
      mockSuccess({});
  ...
  describe('shutdown()', () => {
    ...
    it('should be safe to call multiple times', async () => {
      const exe = createClient();
      await exe.shutdown();
      await exe.shutdown(); // no-op, should not throw
    });

    it('should remain callable when traces are quota-blocked', async () => {
      const exe = createClient();
      (exe as any)._buffer.push({
        traceId: 'tr-shutdown',
      ...
      await expect(exe.shutdown()).resolves.toBeUndefined();
    });
  });
  ...
  describe('checkAgentStatus()', () => {
    it('should return unknown when no agents are registered', async () => {
      const exe = createClient();
      const status = await exe.checkAgentStatus('nonexistent');
  ...
  describe('wrap()', () => {
    it('should trace a successful function execution', async () => {
      ...
      const wrapped = exe.wrap(fn, { agentId: 'bot' });
