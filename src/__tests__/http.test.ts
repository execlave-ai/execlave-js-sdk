...
describe('HTTP request()', () => {
  ...
  describe('successful requests', () => {
    ...
    it('HMAC-signs the body when sign.key is provided', async () => {
      ...
      req.end = jest.fn();
      req.destroy = jest.fn();
      (http.request as jest.Mock).mockImplementation((opts: any, callback: (r: any) => void) => {
        captured = opts;
      ...
      const body = { agentId: 'a', input: 'hi' };
      await request({
        method: 'POST',
        url: 'http://localhost:4000/api/v1/policies/enforce',
        body,
        sign: { key: 'exe_test_key' },
      });

      const ts = captured.headers['X-Execlave-Timestamp'];
      const sig = captured.headers['X-Execlave-Signature'];
      expect(ts).toMatch(/^\d+$/);
      const expected =
        'sha256=' +
        createHmac('sha256', 'exe_test_key')
          .update(`${ts}.${JSON.stringify(body)}`)
          .digest('hex');
      expect(sig).toBe(expected);
    });

    it('does NOT add signature headers when sign is absent', async () => {
      let captured: any;
      const req = new EventEmitter() as any;
      req.write = jest.fn();
      req.end = jest.fn();
      ...
      await request({ method: 'POST', url: 'http://localhost:4000/api/x', body: { a: 1 } });
      expect(captured.headers['X-Execlave-Signature']).toBeUndefined();
    });
