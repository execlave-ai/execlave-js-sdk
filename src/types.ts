...
export interface ExeclaveConfig {
  ...
  /**
   * Called whenever the SDK allows an action WITHOUT a server enforcement
   * decision — i.e. every moment the agent ran ungoverned.
   *
   * Under `fail_open` (the default) an outage is invisible: the call returns
   * `allowed: true` and the only trace was a debug log, which is off in
   * production. That is precisely the window an auditor needs to know about,
   * so route this to your alerting or SIEM rather than to a log file.
   *
   * The handler is called synchronously and its exceptions are swallowed — a
   * faulty listener must never break the enforcement path.
   */
  onEnforcementBypassed?: (event: EnforcementBypassEvent) => void;
...
export interface EnforcePolicyOptions {
  ...
  tools?: string[];
  /**
   * Tool outputs to scan BEFORE they are fed back to the model — the
   * synchronous, preventive side of `tool_output_scan` (PII / injection in a
   * tool's return value). A block-mode policy throws `PolicyBlockedError`, so
   * the caller can stop a poisoned tool result from reaching the agent. Prefer
   * {@link Execlave.enforceToolOutput} for the common single-tool case.
   */
  toolOutputs?: ToolOutput[];
  /**
   * Recent conversation turns. When provided, injection scanning runs over the
   * combined recent user turns plus `input`, so an attack split across turns (a
   * "crescendo") is caught rather than passing because no single turn trips a
   * pattern. Only user-role turns are scanned server-side. Bounded to 50 turns.
   */
...
/** A single conversation turn for multi-turn injection scanning. */
export interface ConversationTurn {
  role: 'user' | 'assistant' | 'system';
  content: string;
}

/** A tool result presented for pre-delivery output scanning. */
export interface ToolOutput {
  name: string;
  /** The tool's input/arguments, for context (optional). */
  input?: unknown;
  /** The tool's return value — scanned for PII / injection before use. */
  output: unknown;
}
...
/** Why an action was allowed without a server enforcement decision. */
export type EnforcementBypassReason =
...
/**
 * Emitted every time enforcement is bypassed and the action proceeds anyway.
 *
 * Deliberately structured rather than a log string: this is compliance
 * evidence — the record of when governance was not applied — and it needs to
 * be machine-routable to an alert or SIEM.
 */
export interface EnforcementBypassEvent {
  reason: EnforcementBypassReason;
  /** Matches `EnforceResult.source` where one is set, for correlation. */
  source: string;
  agentId: string;
  /** ISO-8601, so the ungoverned window can be reconstructed after the fact. */
  timestamp: string;
  /** Human-readable cause, when the SDK has one. */
  message?: string;
  /** Consecutive enforcement failures at the time of the bypass. */
  consecutiveFailures?: number;
  /** HTTP status, when the bypass followed a server response. */
  status?: number;
}
...
export interface EnforceResult {
  ...
  /**
   * Provenance of a locally-resolved decision. Set when the SDK returns
   * `allowed: true` WITHOUT a server enforcement decision — currently
   * `fail_open_network_error` and `fail_open_server_error`. Absent on normal
   * server-sourced results. Lets callers distinguish a governed allow from an
   * ungoverned fail-open allow (e.g. to alert on "enforcement bypassed").
   */
  source?: string;
  semanticCheck?: {
    intent_safe: boolean;
